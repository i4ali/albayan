#!/usr/bin/env python3
"""Consolidate Urdu tafsir fragments into a single per-surah file.

Reads all fragment files matching `new_tafsir/tafsir_<N>_v<start>-<end>_ur.json`
for a given surah and merges them into a single `new_tafsir/tafsir_<N>_ur.json`
suitable for `scripts/merge_urdu_layers.py`.

Usage:
    python3 scripts/consolidate_urdu_fragments.py <surah_number> [--dry-run]

Example:
    python3 scripts/consolidate_urdu_fragments.py 3
    python3 scripts/consolidate_urdu_fragments.py 3 --dry-run
"""
...
FRAGMENT_RE = re.compile(r"^tafsir_(\d+)_v(\d+)-(\d+)_ur\.json$")
...
def consolidate(surah_num: int, dry_run: bool = False):
    ...
    output_path = fragments_dir / f"tafsir_{surah_num}_ur.json"
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 consolidate_urdu_fragments.py <surah_number> [--dry-run]")
        sys.exit(1)
